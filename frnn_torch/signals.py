...
edens_profile_thomson = ProfileSignal(
    ...
    mapping_paths=[ None], causal_shifts=[2],
...
qmin = Signal(
    ...
    [d3d], causal_shifts=[10], normalize=False,
...
# superset of shots.
ip = Signal("plasma current ipsip", ["jpf/da/c2-ipla", "/ipsip"],
            [jet, d3d], is_ip=True)
...
n1_rms_no_shift = Signal("n1 finite frequency signals",['mhd/mirnov.n1rms'],[d3d],data_avail_tolerances=[0.029])
...
pin = Signal("Input Power (beam for d3d)", ['jpf/gs/bl-ptot<s', '/bmspinj'],
             [jet, d3d])
pint = Signal("Input Power (beam for d3d) tol", ['jpf/gs/bl-ptot<s', '/bmspinj'],
             [jet, d3d],data_avail_tolerances=[0.029, 0.029])
...
all_signals_real_time_0D={
    ...
    'energydt': energydt,'iperr':iperr, 'ipdirect': ipdirect, 'iptarget': iptarget
    # 'tmamp1':tmamp1, 'tmamp2':tmamp2, 'tmfreq1':tmfreq1, 'tmfreq2':tmfreq2,
    # 'pechin':pechin,
    # 'rho_profile_spatial':rho_profile_spatial, 'etemp':etemp,
}
